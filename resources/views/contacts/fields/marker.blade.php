<div class="input-group hidden default-div default-hidden-div" style="margin-bottom: 15px;">
    <div class="form-group111  my-map-group map-control ">
        {{Form::select('markers[country_id][]', $countries->pluck('name', 'id') , 1, ['class'=>'form-control col-sm-12 myselect-country country', 'placeholder'=>_('Select country..')] )}}
        {{Form::select('markers[city_id][]', [] , null, ['class'=>'form-control col-sm-12 myselect-city city', 'placeholder'=>_('Select city..'), ''] )}}

        {{Form::text('markers[address][]', '', ['class'=>'form-control address', 'disabled'])}}
        {{Form::text('markers[marker_zip][]', null, ['class'=>'form-control zip', 'disabled', 'placeholder'=>'zip'])}}

        {{Form::hidden('markers[address_id][]', null, ['class'=>'adress_id'])}}
        {{Form::hidden('markers[map_center][]', null, ['class'=>'map_center'])}}
        {{Form::hidden('markers[map_zoom][]', null, ['class'=>'map_zoom'])}}
        {{Form::hidden('markers[marker_position][]', null, ['class'=>'marker_position'])}}

        {{--{{Form::hidden('markers[marker_city][]', null, ['class'=>'marker_city'])}}--}}
        {{Form::hidden('markers[marker_address][]', null, ['class'=>'marker_address'])}}
        {{--{{Form::hidden('markers[marker_zip][]', null, ['class'=>'marker_zip'])}}--}}
...
{{-- te izvadīsim esošās adrese ->kartes->markerus --}}
{{--{{dd($addresses)}}--}}
...
@if(isset($addresses))
    @foreach($addresses as $address)
        <div class="input-group default-div"  style="margin-bottom: 15px;">
            <div class="form-group111  my-map-group map-control">
                {{Form::select('markers[country_id][]', $countries->pluck('name', 'id') , isset($address->country_id) ? $address->country_id : null, ['class'=>'form-control col-sm-12 myselect-country country', 'placeholder'=>_('Select country..')] )}}
                {{Form::select('markers[city_id][]', isset($address->country->cities) ? $address->country->cities->pluck('name', 'id') : null, isset($address->city_id) ? $address->city_id : null, ['class'=>'form-control col-sm-12 myselect-city city', 'placeholder'=>_('Select city..'), isset($address->city_id) ? :'disabled'] )}}

                {{Form::text('markers[address][]',isset($address->marker_address) ? $address->marker_address : null, ['class'=>'form-control address', isset($address->marker_address) ? :'disabled'])}}
                {{Form::text('markers[marker_zip][]', isset($address->marker_zip) ? $address->marker_zip : null, ['class'=>'form-control zip', isset($address->marker_zip) || isset($address->marker_address) ? :  'disabled'])}}

                {{Form::hidden('markers[address_id][]', $address->id, ['class'=>'adress_id'])}}
                {{Form::hidden('markers[map_center][]', $address->map_center, ['class'=>'map_center'])}}
                {{Form::hidden('markers[map_zoom][]', $address->map_zoom, ['class'=>'map_zoom'])}}
                {{Form::hidden('markers[marker_position][]', $address->marker_position, ['class'=>'marker_position'])}}

                {{--{{Form::hidden('markers[marker_city][]', $address->marker_city, ['class'=>'marker_city'])}}--}}
                {{Form::hidden('markers[marker_address][]', $address->marker_address, ['class'=>'marker_address'])}}
                {{--{{Form::hidden('markers[marker_zip][]', $address->marker_zip, ['class'=>'marker_zip'])}}--}}
                <div class="map" style="width: 100%;height: 300px;"></div>
            </div>
            <span class="input-group-addon remove-button btn-danger"><i class="fa-remove"></i></span>
        </div>
...
{{-- beidzam izvadīt esoša kartes, markerus--}}
<script>
...
@if( !isset($googleMapFileIsLoaded) )
